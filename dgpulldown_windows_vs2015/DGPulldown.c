...
BOOL CALLBACK DlgProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lParam)
{
	...
	switch(Message)
	{
		case WM_INITDIALOG:
			...
			pOldEditProc = SetWindowLong(GetDlgItem(hwnd, IDC_INPUT), GWLP_WNDPROC, (LONG)NewEditSubWindowProc);
			break;
...
//					strcat(output_file, ".pulldown.m2v");
			...
			break;
		case WM_CLOSE:
			SetWindowLong(GetDlgItem(hwnd, IDC_INPUT), GWLP_WNDPROC, (LONG)pOldEditProc);
			EndDialog(hwnd, 0);
			break;
		default:
			return FALSE;
