##
if (NOT Z_BUILD_CMAKE_COMPILER_FLAGS_CMAKE_)
set(Z_BUILD_CMAKE_COMPILER_FLAGS_CMAKE_ 1)

include(CheckCCompilerFlag)
include(CheckCXXCompilerFlag)

# Strings used to cache failed C/CXX flags.
set(Z_FAILED_C_FLAGS)
set(Z_FAILED_CXX_FLAGS)

# Checks C compiler for support of $c_flag. Adds $c_flag to $CMAKE_C_FLAGS when
# the compile test passes. Caches $c_flag in $Z_FAILED_C_FLAGS when the test
# fails.
function (add_c_flag_if_supported c_flag)
  ...
  string(FIND "${Z_FAILED_C_FLAGS}" "${c_flag}" C_FLAG_FAILED)
  ...
  if (${C_FLAG_FOUND} EQUAL -1 AND ${C_FLAG_FAILED} EQUAL -1)
    ...
    if (C_FLAG_SUPPORTED)
      ...
      set(Z_FAILED_C_FLAGS "${Z_FAILED_C_FLAGS} ${c_flag}" CACHE STRING ""
          FORCE)
    endif ()
  endif ()
endfunction ()
...
# $Z_FAILED_CXX_FLAGS when the test fails.
function (add_cxx_flag_if_supported cxx_flag)
  ...
  string(FIND "${Z_FAILED_CXX_FLAGS}" "${cxx_flag}" CXX_FLAG_FAILED)
  ...
  if (${CXX_FLAG_FOUND} EQUAL -1 AND ${CXX_FLAG_FAILED} EQUAL -1)
    ...
    if (CXX_FLAG_SUPPORTED)
      ...
      set(Z_FAILED_CXX_FLAGS "${Z_FAILED_CXX_FLAGS} ${cxx_flag}" CACHE
          STRING "" FORCE)
    endif ()
  endif ()
endfunction ()
...
function (append_as_flag flag)
  ...
  string(FIND "${Z_AS_FLAGS}" "${flag}" AS_FLAG_FOUND)

  if (${AS_FLAG_FOUND} EQUAL -1)
    set(Z_AS_FLAGS "${Z_AS_FLAGS} ${flag}" CACHE STRING "" FORCE)
  endif ()
endfunction ()
...
endif ()  # Z_BUILD_CMAKE_COMPILER_FLAGS_CMAKE_
